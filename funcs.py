...
def visualizar_mapa(mapa, centros=None, raio=None, rota=None, distancia=None, percentual=None, num_circulos=None):
    ...
    if rota is not None and centros is not None:
        ...
        # Rota
        plt.plot(rota_y, rota_x, color='red', linewidth=2, marker='o', label='Rota')
        
        plt.plot(rota_y[0], rota_x[0], 'o', color='lime', markersize=10, label='Início')


    # Monta legenda
    plt.title(f'MELHOR RESULTADO\nCobertura: {percentual:.2f}% | Pontos: {num_circulos} | Distância: {distancia:.2f}', fontsize=12)
    plt.axis('off')
    plt.show()
